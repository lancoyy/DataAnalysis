 ...
 * in Journal of the ACM 46 (5): 604鈥�632 (1999)
 *
 * @author pjmcswee
 */
public  class Hits implements Statistics, LongTask ,ImportantNode{
    ...
    public  ArrayList<Nodes> execute(ArrayList<Edges> edgesInfos,HierarchicalGraph hgraph, AttributeModel attributeModel) {
        ...
        authority = new double[N];//权威值
        hubs = new double[N];//枢纽值
...
//            System.out.println(s.getNodeData().getLabel()+":权威值"+authority[index1]+"..枢纽值"+hubs[index1]);
//        }
        ...
        for (Node s : hgraph.getNodes()) {
          ...
          //  System.out.println(authorityCol+"  *****  "+(float) authority[s_index]+"  *****   "+hubsCol+"   *******   "+(float) hubs[s_index]);
            Nodes node=new Nodes();
	           ...
	           node.setNoteWeight((float) (authority[s_index]+hubs[s_index])+"");
    ...
    public String getReport() {
        ...
        String report = "<HTML> <BODY> <h1> HITS Metric Report </h1>"
                ...
                + "Jon M. Kleinberg, <i>Authoritative Sources in a Hyperlinked Environment</i>, in Journal of the ACM 46 (5): 604鈥�632 (1999)<br />"
                + "</BODY> </HTML>";
